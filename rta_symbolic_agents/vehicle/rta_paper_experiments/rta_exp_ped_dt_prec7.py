...
printResults = "rta_exp_ped_dt_prec7-" + today + ".txt" 
...
with open(printResults,'w') as f:
  ...
  asys = "asysPedXLine(3,6/1,4/1,2/1,1/10)"
  print("Maude Experiment:\n  " + maudeLoad + "\n  " + maudeModule + "\n  " + asys,file=f)
  start = timer()
  results = checkDtPrec1OneLog(maudeLoad,maudeModule,"saferSP","safeSP",6,4,2,asys,f)
  end = timer()
  delta = end - start 
